...
source('~/Path/r-compile/rUtils/R/rUtils.R')
source('~/Path/r-compile/Path/R/Path.R')
source('~/Path/Project/r-files/functions/functions_Project.R')
source('~/Path/Project/r-filesUser/load_Project_Human.R')
...
for (subid in 1:length(Subject_list)) {
  ...
  if (strcmp(Generate_EEG_plots, 'YES')) {
    ...
    if (!file.exists(paste("~/Path/Project/r-filesUser/Figures/Model_fit/EEG/",Subject, "_EEG_NA_NA.png",sep="")) || strcmp(Overwrite_plots, 'YES')){
    for (w in 1:length(Wave_list)) {
      ...
      if (file.exists(paste("/Volumes/Path/Path/Project/Path/PathFit",Subject, "Project",Subject,Parcel,Hemisphere,Wave,"gd.rda",sep="_"))){
        ...
        tn <- load_Project_Human(Wave, Subject, Type_signal, Hemisphere,Parcel)
        # Get the structure
        ST <- tn[['xpp']]
        ...
        # Load the model fit variable (replaced with the calPath)
        ...
        tst <- callPath(tn, DVstr='DV', gd=T, show=TRUE, showCh=1,FUN=Project,maxISI=10, minISI=0.2, maxp2p=5000, DVsdscale=5, 
    ...
    ggsave(
      ...
      plot = Final_plot_sub,
      device = "png",
      path = "~/Path/Project/r-filesUser/Figures/Model_fit/EEG/Project",
      scale = 2,
      width = 15,
      height = 5,
      units = "cm",
  ...
  if (strcmp(Generate_source_plots, 'YES')) {
    ...
    for (parc in 1:length(Parcel_list)) {
      ...
      for (hem in 1:length(Hemisphere_list)) {
        ...
        if (!file.exists(paste("~/Path/Project/r-filesUser/Figures/Model_fit/Sources/",Subject, "_Source_",Parcel,"_",Hemisphere,".png",sep=""))){
        for (w in 1:length(Wave_list)) {
          Wave <- Wave_list[w]
        ...
        ggsave(
          ...
          plot = Final_plot_sub,
          device = "png",
          path = "~/Path/Project/r-filesUser/Figures/Model_fit/Sources/Project",
          scale = 2,
          width = 15,
          height = 5,
          units = "cm",
