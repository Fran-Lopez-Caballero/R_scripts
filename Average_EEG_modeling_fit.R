...
source('~/Path/Path/Path/R/Path.R')
source('~/Path/Path/Path/R/Path.R')
source('~/Path/Project/r-files/functions/functions_Project.R')
source('~/Path/Project/Path/load_Project_Human.R')

Generate_EEG_plots <- "YES"
Generate_source_plots <- "NO!" # Average not programmed for this yet
Overwrite_plots <- "YES" # If willing to repeat and overwrite pPathent plots
  ...
  All_se_pred <- matrix(list(), length(Subject_list), 15)
      ...
      for (subid in 1:length(Subject_list)) {
        ...
        if (file.exists(paste("/Volumes/Path/Path/Project/Path/PathFit",Subject, "Project",Subject,Parcel,Hemisphere,Wave,"gd.rda",sep="_"))){
          ...
          ST$Pred <- tst$Path$lm2$fitted.values
          # Extract fit values
      ...
      save(list=c("ISI_means_real", "ISI_se_real", "INT_means_real","INT_se_real","All_means_real","All_se_real","ISI_means_pred","ISI_se_pred","INT_se_pred","All_means_pred","All_se_pred"), file = paste("~/Path/Project/Path/Summary_table_fits/",Type_signal,"_",Parcel,"_",Hemisphere,"_",Wave,sep=""))
...
ggsave(
  ...
  plot = Final_plot_sub,
  device = "png",
  path = "~/Path/Project/Path/Figures/Model_fit/EEG/Project/",
  scale = 2,
  width = 15,
  height = 5,
  units = "cm",
